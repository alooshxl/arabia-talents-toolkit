...
const AIVideoSummarizer = () => {
  ...
  useEffect(() => {
    // If the specific error "YouTube API Key is not configured..." is currently displayed,
    // and the youtubeApiKey (from context) is now available (truthy),
    // then clear the error.
    if (youtubeApiKey && error === 'YouTube API Key is not configured. Please set it in the application settings.') {
      setError(null);
    }
    // Optional: If the key is suddenly missing and a different error (or no error) is shown,
    // this part could set the specific error. However, the existing UI
    // `{!youtubeApiKey && (<Alert variant="destructive" ...>)}`
    // should already handle displaying the error when the key is missing during render.
    // So, primarily, this useEffect is for clearing the error when the key appears.
  }, [youtubeApiKey, error, setError]); // Dependencies
